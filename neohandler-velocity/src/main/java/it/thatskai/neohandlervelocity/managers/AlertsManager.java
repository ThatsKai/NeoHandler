package it.thatskai.neohandlervelocity.managers;
...
public class AlertsManager {
    ...
    public void sendAlert(String s){
        for(Player player : NeoHandlerVelocity.getInstance().getProxyServer().getAllPlayers()){
            if(player.hasPermission("neohandler.alerts") && StaffUtils.playerAlerts.contains(player.getUniqueId())){
    ...
    public void handleGrimAlert(String var1){
        ...
        Optional<Player> p = NeoHandlerVelocity.getInstance().getProxyServer().getPlayer(player);
        if(p.isPresent()){
        ...
        if(NeoHandlerVelocity.getInstance().isDatabaseConnect()){
            NeoHandlerVelocity.getInstance().getSqlProvider().getLogsTable().addLog(player,ConfigCache.ACLOGS_GRIM_FORMAT
                    .replace("%server%",server)
                    .replace("%player%",player)
                    .replace("%vl%",vl+"")
                    .replace("%check%",check)
    ...
    public void handleVulcanAlert(ByteArrayDataInput var1){
        ...
        if(NeoHandlerVelocity.getInstance().isDatabaseConnect()){
            NeoHandlerVelocity.getInstance().getSqlProvider().getLogsTable().addLog(player,ConfigCache.ACLOGS_VULCAN_FORMAT
                    .replace("%server%",server)
                    .replace("%player%",player)
                    .replace("%vl%",vl+"")
                    .replace("%check%",checkName)
    ...
    public void handleBan(String player, String check, double vl){
        for(Player p : NeoHandlerVelocity.getInstance().getProxyServer().getAllPlayers()){
            if(p.hasPermission("neohandler.showban")){
